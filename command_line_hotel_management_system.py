...
mydb = mysql.connector.connect(
    ...
    user="root",
    password="",
    database="ayush_hotels_cli"
)
mycursor = mydb.cursor()


def menu():
    print("Welcome to Ayush Hotels")
...
def addcust():
    ...
    val = (name, address, contact, email)
    # print(val)
...
def additem():
    print("Item Entry")
    roomname = input("Enter item type : ")
    price = input("Enter item price (Rs) : ")
    roomnumber = input("Enter total number of rooms : ")
...
def checkstock():
    ...
    for x in myitem:
        ...
        if (int(x[3]) < 10):
            ...
            server.login("ayushojha010@gmail.com", password)
    ...
    if (res == True):
        ...
        for i in myitem:
            ...
            if (i[1] == item):
                if (int(quantity) <= int(i[3])):
                    ...
                    mycursor.execute(
                        "UPDATE item SET itemnumber = '" + str(temp) + "'WHERE itemid = " + str(i[0]) + ";")
                    mydb.commit()
                else:
                    print("Item remaining in stock : " + i[3] + " and item customer buying is : " + item)
        ...
        for i in myrecord:
            ...
            if (i[1] == name):
                ...
                mycursor.execute("UPDATE supplydetail SET item='" + tempitem + "',money='" + str(
                    tempmoney) + "',stat='" + stat + "'  WHERE supplyid = " + str(i[0]) + ";")
                mydb.commit()
            else:
                stat = custstat(totalprice)
                tempitem = item + "*" + quantity
                mycursor.execute(
                    "INSERT INTO supplydetail (custname, item, money,stat) VALUES ('" + name + "','" + tempitem + "','" + str(
                        totalprice) + "','" + stat + "');")
                mydb.commit()
